...
describe('getPositionByPlayerAndIndex()', () => {
  ...
  it('should throw playerLength error', () => {
    function playerLengthTest1() {
      getPositionByPlayerAndIndex(1, 0);
    }

    expect(playerLengthTest1).toThrowError(
      new Error('playerLength must be greater than or equal 2 and less than or equal 10.')
    );

    function playerLengthTest2() {
      getPositionByPlayerAndIndex(11, 0);
    }

    expect(playerLengthTest2).toThrow(
      new Error('playerLength must be greater than or equal 2 and less than or equal 10.')
    );
  });
  ...
  it('should throw index error', () => {
    ...
    expect(indexTest1).toThrow(new Error('Index number must be greater than or eaual 0 and less than playerLength(2)'));

    function indexTest2() {
      getPositionByPlayerAndIndex(2, 10);
    }

    expect(indexTest2).toThrow(new Error('Index number must be greater than or eaual 0 and less than playerLength(2)'));
  });
});
